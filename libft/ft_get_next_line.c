...
/*   Updated: 2019/04/28 22:36:10 by ilya             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "libft.h"

static int					del(t_dlist *begin, char **a, int fd)
{
	t_dlist	*me;
	t_dlist *prev;
...
static int					cut(t_dlist *begin, char *str, int i, int fd)
{
	void			*a;
...
static void					*manage_line(char **line, int *c, char *str, int i)
{
	void			*a;
	char			*ln;
...
static int					do_something(int fd, char **line, t_dlist *begin)
{
	t_dlist			*me;
	size_t			n;
	...
	while ((me = me->next))
		if (me->label == fd)
		{
			...
			if ((n != me->size) && ((char*)(me->data))[n] == '\n')
			{
				...
				ft_memmove(me->data,
				&(((char*)(me->data))[n + 1]), me->size - n - 1);
				me->size = me->size - n - 1;
				return (*line == NULL ? del(begin, 0, fd) : -2);
			}
			ft_smemmove(*line = ft_strnew(n), me->data, n == 0 ? 0 : n + 1);
			me->size = 0;
			return (*line == NULL ? del(begin, 0, fd) : (int)n);
		}
	while (begin->next)
		begin = begin->next;
	return ((begin->next = ft_dlstadd(fd, NULL)) == NULL ? -2 : 0);
}
